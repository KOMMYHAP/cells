...
class YamlSystem:
    ...
    def get_jinja_components(self):
        components = []
        for component in self.components:
            components.append({
                'file_name': component.get_cpp_filename(),
                'class_name': component.get_cpp_structure_name(),
                'argument_type_and_name': component.get_argument_type_and_name()
            })
        return components
...
def _generate_systems_headers(environment, systems: list[YamlSystem], output_directory):
    ...
    for system in systems:
        content = template.render(
            system=system.get_jinja_system(),
            components=system.get_jinja_components()
        )
