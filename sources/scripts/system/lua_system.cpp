    ...
    , _menuRootWidget(&menuRootWidget)
{
    ...
    _luaState.new_usertype<LuaMenu>("menu",
        ...
        "root_widget", std::to_underlying(MenuWidgetId::Root),
        "invalid_widget", std::to_underlying(MenuWidgetId::Invalid));
...
{
    sol::function_result result = _luaState.script(script, sol::default_traceback_error_handler);
    if (!result.valid()) {
